...
export default function Home() {
  ...
  useEffect(() => {
    const fetchOfferListings = async () => {
      try {
        const res = await fetch(`https://real-estate-api-ashy.vercel.app/api/listing/search?offer=true&limit=3`);
    ...
    const fetchRentListings = async () => {
      try {
        const res = await fetch(`https://real-estate-api-ashy.vercel.app/api/listing/search?type=rent&limit=3`);
        const data = await res.json();
        setRentLisingd(data);
        fetchSaleListings();
      } catch (error) {
    ...
    const fetchSaleListings = async () => {
      try {
        const res = await fetch(`https://real-estate-api-ashy.vercel.app/api/listing/search?type=sale&limit=3`);
        const data = await res.json();
  ...
  return (
    <div className='px-10'>
      ...
      <div className='flex flex-col gap-6 p-20 px-3 max-w-6xl mx-auto'>
        ...
        <Link to={'/search'} className='text-xs sm:text-sm text-blue-800 hover:underline font-bold'>
